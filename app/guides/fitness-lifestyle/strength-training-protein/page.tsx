...
export const metadata = {
  ...
  description:
    "러닝과 근력운동을 병행할 때 필요한 단백질 전략, 회복 타이밍, 주간 총량 기준을 정리했습니다.",
};

const focusRows = [
  ["근력운동 직후", "20~40g", "근육 회복과 다음 세션 준비를 위한 기본 범위입니다."],
  ["러닝 병행 주간", "1.6~2.0 g/kg/day", "지구력과 근력 모두의 회복을 고려한 총량 기준입니다."],
  ["감량기 근손실 방어", "2.0 g/kg/day 안팎", "총열량이 낮을수록 단백질 유지가 더 중요합니다."],
];

const liftCards = [
  {
    title: "근력운동 후 1시간",
    body: "러닝과 달리 근력운동 주간에는 운동 후 회복 창을 놓치지 않는 것이 중요합니다. 탄수화물과 단백질을 함께 넣으면 체감 회복도 좋아집니다.",
  },
  {
    title: "주간 총량 유지",
    body: "근력운동이 2~3회뿐이어도 총 단백질 섭취량이 부족하면 근육 유지가 어렵습니다. 운동일뿐 아니라 휴식일 총량도 같이 봐야 합니다.",
  },
  {
    title: "러닝과 병행 시 우선순위",
    body: "러닝 위주 루틴에 근력운동이 들어가면 회복 부담이 커집니다. 고강도 주간은 단백질과 탄수화물을 함께 설계하는 편이 안정적입니다.",
  },
];

const sampleRows = [
  ["아침", "계란·그릭요거트·토스트", "단백질과 탄수화물로 하루 시작"],
  ["운동 후", "쉐이크 또는 RTD 단백질 음료", "빠른 회복 연결"],
  ["저녁", "생선·닭고기·밥", "총량 확보와 야간 회복 대비"],
];

const scheduleRows = [
  ["러닝만 하는 날", "러닝 후 20~30g", "회복과 다음 훈련 준비"],
  ["근력운동만 하는 날", "운동 후 20~40g", "근육 회복과 자극 유지"],
  ["러닝+근력 병행일", "운동 후 30g 안팎 + 평소 식사 유지", "두 세션 회복 부담을 동시에 고려"],
...
const checklist = [
  ...
  "감량기라면 단백질 밀도가 높은 제품을 우선 보기",
  "고강도 하체 운동 다음 날 러닝이 있다면 회복식 간격까지 설계하기",
];
      ...
      <section className="w-full border-t border-b bg-[var(--hero-bg)]" style={{ borderColor: "var(--hero-border)" }}>
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            ...
            <br />
            근력운동이 들어가는 주간은 총량과 운동 직후 회복식을 더 신경 써야 합니다.
          </p>
        </div>
      </section>
        ...
        <div className="space-y-6">
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-[#eff7f1] px-3 py-1 text-xs font-semibold text-[#2d6a4f]">회복과 유지</span>
            </div>
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--foreground-muted)]">
              근력운동을 병행한다면 운동 직후만 챙기는 방식보다, 아침부터 저녁까지 포함한 총량을 안정적으로 채우는 흐름이 좋습니다.
            </p>
            <div className="mt-5 overflow-x-auto rounded-2xl border border-[#ece9e2] bg-white">
              <table className="min-w-full border-collapse text-left text-sm">
                <thead>
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--foreground-muted)]">
              러닝과 근력운동을 같이 하는 사용자는 운동 종류보다 주간 훈련 전체를 기준으로 섭취 전략을 보는 편이 더 실전적입니다.
            </p>
            <div className="mt-5 overflow-x-auto rounded-2xl border border-[#ece9e2] bg-white">
              <table className="min-w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-[#e8e6e3] text-[var(--foreground)]">
                    <th className="px-3 py-3 font-semibold">상황</th>
                    <th className="px-3 py-3 font-semibold">권장 사인</th>
                    <th className="px-3 py-3 font-semibold">의미</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {checklist.map((item) => (
                <label key={item} className="flex items-start gap-3 rounded-2xl border border-[#dce8df] bg-white px-4 py-4 text-sm leading-6 text-[var(--foreground-muted)] shadow-[0_10px_26px_rgba(45,106,79,0.05)]">
          ...
          <section className="rounded-[28px] border border-[#e8e6e3] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            <div className="flex flex-wrap gap-3">
              <Link href="/guides/fitness-lifestyle/running-protein-guide" className="inline-flex items-center justify-center rounded-xl border border-[#d9e7dc] bg-white px-5 py-3 text-sm font-semibold text-[#24543d] transition-colors hover:bg-[#eef7f1]">
                러닝 단백질 가이드 보기
              </Link>
              <Link href="/guides/fitness-lifestyle/sports-nutrition-guide" className="inline-flex items-center justify-center rounded-xl border border-[#2d6a4f] bg-[#2d6a4f] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#24543d]">
                스포츠 영양 가이드 보기
              </Link>
            </div>
          </section>
