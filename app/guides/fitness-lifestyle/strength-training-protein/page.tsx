...
const focusRows = [
  ...
  ["러닝 병행 주간", "1.6~2.0 g/kg/day", "지구력과 근력 운동을 함께 고려한 총량 기준"],
  ["감량기 근손실 방어", "2.0 g/kg/day 전후", "총열량이 낮을수록 단백질 비중을 더 봅니다."],
];

const liftCards = [
  {
    title: "근력운동 후 1시간",
    body: "러닝과 함께 근력운동도 하는 주간이라면 운동 후 회복 창을 놓치지 않는 편이 좋습니다. 필수아미노산과 단백질을 같이 챙기면 회복 체감이 더 안정적입니다.",
  },
  {
    title: "주간 총량 유지",
    body: "근력운동을 주 2~3회 이상 한다면 총 단백질 섭취가 부족하지 않은지 먼저 확인해야 합니다. 운동일뿐 아니라 휴식일도 중요합니다.",
  },
  {
    title: "러닝과 병행할 때 우선순위",
    body: "러닝과 근력운동을 동시에 하는 주간은 회복 부담이 커집니다. 주간 전체 식사 흐름 안에서 단백질과 필수 탄수화물을 함께 넣는 편이 안정적입니다.",
  },
];

const sampleRows = [
  ["아침", "계란, 그릭요거트, 우유", "단백질과 탄수화물로 하루 시작"],
  ["운동 후", "쉐이크 또는 RTD 단백질 음료", "빠른 회복 루틴 연결"],
  ["저녁", "생선, 두부, 닭가슴살", "총량 보완과 근육 회복 대비"],
];

const scheduleRows = [
  ["러닝만 하는 날", "운동 후 20~30g", "회복과 다음 훈련 준비"],
  ["근력운동만 하는 날", "운동 후 20~40g", "근육 회복과 적응 유지"],
  ["러닝+근력 병행", "운동 후 30g 전후 + 식사 보강", "피로 누적을 함께 고려"],
  ["휴식일", "식사별 총량 유지", "주간 평균 섭취 안정화"],
];

const checklist = [
  "근력운동 후 단백질만 보지 말고 필수 탄수화물도 함께 챙기기",
  "러닝 병행 주간에는 휴식일에도 총 섭취량을 유지하기",
  "감량기라면 단백질 비중이 높은 제품을 먼저 보기",
  "고강도 하체 운동 다음 날은 회복 간격까지 같이 설계하기",
];
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {liftCards.map((card) => <InsightCard key={card.title} title={card.title} body={card.body} />)}
            </div>
          </section>
          ...
          <section className="rounded-[28px] border border-[#e8e6e3] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            <div className="flex flex-wrap gap-3">
              <Link href="/guides/fitness-lifestyle/running-protein-guide" className="inline-flex items-center justify-center rounded-xl border border-[#d9e7dc] bg-white px-5 py-3 text-sm font-semibold text-[#24543d] transition-colors hover:bg-[#eef7f1]">러닝 단백질 가이드 보기</Link>
              <Link href="/guides/fitness-lifestyle/sports-nutrition-guide" className="inline-flex items-center justify-center rounded-xl border border-[#2d6a4f] bg-[#2d6a4f] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#24543d]">스포츠 영양 가이드 보기</Link>
            </div>
          </section>
        </div>
      </main>
