...
export const metadata = {
  ...
  description:
    "5km, 10km, 하프, 풀 마라톤까지 거리별 특성에 맞는 훈련 포인트와 단백질·탄수화물 전략을 정리했습니다.",
};

const distanceRows = [
  ["5km", "속도와 인터벌 적응", "낮음", "기록 향상과 스피드 중심 러너"],
  ["10km", "지구력과 페이스 유지", "중간", "스피드와 지구력을 함께 키우는 러너"],
  ["하프", "롱런과 페이스 훈련 비중 증가", "높음", "장거리 적응을 시작하는 러너"],
  ["풀", "장시간 에너지 관리와 회복 최적화", "매우 높음", "완주 또는 기록 향상을 노리는 러너"],
];

const distanceCards = [
  {
    title: "5km 전략",
    training: ["짧은 인터벌", "속도 훈련", "5km 페이스 적응"],
...
const proteinReasons = [
  "거리 훈련이 길어질수록 근육 회복 속도와 다음 훈련 적응이 중요해집니다.",
  "장거리 러닝 이후에는 전체 피로가 커서 회복 루틴을 따로 챙기는 편이 좋습니다.",
  "운동 후 20~30g 단백질 섭취는 회복 루틴을 만들기에 가장 실전적인 기준입니다.",
];
      ...
      <section className="w-full border-t border-b bg-[var(--hero-bg)]" style={{ borderColor: "var(--hero-border)" }}>
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            ...
            <br />
            거리별 특성을 먼저 이해하면 러닝 후 단백질 섭취와 회복 루틴도 더 명확하게 설계할 수 있습니다.
          </p>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-[1200px] px-4 py-8 md:px-6">
        <div className="space-y-6">
          <section className="rounded-[28px] border border-[#e2ebe4] bg-[#f7fbf8] px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            <h2 className="text-xl font-bold text-[var(--foreground)]">거리별 핵심 특성</h2>
              ...
              {distanceCards.map((item) => (
                <article key={item.title} className="rounded-2xl border border-[#dce8df] bg-[#f6fbf7] p-4">
                  ...
                  <div className="mt-4 rounded-xl border border-[#dce8df] bg-white px-3 py-3 text-sm leading-6 text-[var(--foreground-muted)]">
                    <p>
                      <span className="font-semibold text-[var(--foreground)]">훈련 전</span>
                      {" : "}
                      {item.nutritionBefore}
                    </p>
                    <p className="mt-1">
                      <span className="font-semibold text-[var(--foreground)]">훈련 후</span>
                      {" : "}
                      {item.nutritionAfter}
                    </p>
                  </div>
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            <h2 className="text-xl font-bold text-[var(--foreground)]">러닝 후 단백질이 중요한 이유</h2>
            <blockquote className="mt-4 rounded-xl border border-[#dce8df] bg-[#f7fbf8] px-4 py-4 text-sm leading-6 text-[var(--foreground-muted)]">
              러닝 직후 20~30g 단백질 섭취는 회복, 피로 관리, 다음 훈련 적응을 정리하는 가장 실전적인 출발점입니다.
            </blockquote>
