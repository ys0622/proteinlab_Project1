...
export const metadata = {
  ...
  description:
    "감량기나 운동 병행 상황에서 근육량을 지키려면 주간 총량과 분산 섭취를 어떻게 가져가야 하는지 정리한 가이드입니다.",
};

const maintenancePoints = [
  "감량기라면 운동 한 번보다 주간 총량을 먼저 맞추는 것이 더 중요합니다.",
  "운동하지 않는 날에도 단백질 루틴을 유지하는 편이 근육 유지에 유리합니다.",
  "제품은 식사를 대체하기보다 식사 계획을 보완하는 도구로 보는 쪽이 안정적입니다.",
];

const maintenanceMatrix = [
  ["감량기", "총량 유지", "체중 감량 중에도 근손실을 줄이려면 우선 주간 총량이 무너지지 않아야 합니다."],
  ["근력 운동 병행", "운동 후 + 하루 분산", "회복용 보충과 전체 루틴을 같이 챙겨야 유지 효율이 올라갑니다."],
  ["바쁜 일정", "간식 루틴 확보", "오전이나 오후 간식을 보완 지점으로 만들면 총량 유지가 쉬워집니다."],
];
      ...
      <section className="w-full border-t border-b bg-[var(--hero-bg)]" style={{ borderColor: "var(--hero-border)" }}>
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            감량 중이든 운동을 병행하든 단백질 루틴을 안정적으로 유지하는 것이 근육 유지의 기본입니다.
          </p>
        </div>
      </section>
      <main className="mx-auto max-w-[1200px] px-4 py-8 md:px-6">
        <section className="rounded-[28px] border border-[#e2ebe4] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
          <div className="rounded-2xl border border-[#dce8df] bg-[#f7fbf8] px-4 py-4">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--foreground-muted)]">
              근육 유지 전략은 운동 직후 한 번이 아니라 하루 총량을 반복적으로 채우는 루틴입니다.
            </p>
          </div>
        ...
        <section className="mt-6 rounded-[28px] border border-[#e2ebe4] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--foreground-muted)]">
            근육 유지 전략은 고단백 제품을 많이 먹는 것이 아니라, 주간 총량을 흔들리지 않게 보완 지점을 만드는 것에서
            시작합니다.
          </p>
        </section>
      </main>
