...
const rows = [
  ["단백질만 높은 RTD", "보충용에 가까움", "칼로리와 포만감이 낮아 한 끼 대체보다는 부족한 부분을 보완하는 쪽에 가깝습니다."],
  ["균형 영양형 음료", "식사 보완형", "칼로리와 영양 구성이 더 붙어 있어 음료 중에서는 식사 대체에 가장 가깝습니다."],
  ["파우치형 쉐이크", "식사 대용에 유리", "음료만으로 버티기 어렵다면 결국 쉐이크 쪽으로 넘어가는 편이 자연스럽습니다."],
];

const steps = [
  "한 끼를 완전히 대체하려면 단백질 숫자보다 포만감과 칼로리를 먼저 봐야 합니다.",
  "아침처럼 짧은 대체라면 가벼운 제품도 가능하지만, 점심·저녁 대체는 식사 보완형이 더 맞습니다.",
  "단순 RTD를 식사 대신 계속 마시면 결국 허기가 빨리 와서 간식을 찾게 되는 경우가 많습니다.",
];
...
const mistakes = [
  "단백질 숫자만 보고 식사대용이 된다고 생각하는 경우",
  "가벼운 보충용 RTD를 아침·점심 전체 대체로 계속 마시는 경우",
  "포만감이 부족한데도 간식으로 메우면서 총칼로리가 더 올라가는 경우",
];

const cases = [
  {
    title: "아침 대체",
    body: "짧게 공복을 막는 용도라면 가벼운 제품도 가능하지만, 오전 내내 버틸 수 있는지까지 같이 봐야 합니다.",
  },
  {
    title: "점심 보완",
    body: "회의나 이동으로 식사를 못 챙기는 날이라면 음료보다 포만감이 더 붙는 쉐이크가 더 잘 맞을 수 있습니다.",
  },
  {
    title: "다이어트용 대체",
    body: "체중 관리 목적이라도 너무 가벼우면 다시 간식으로 돌아가기 쉽습니다. 총칼로리보다 지속성이 중요합니다.",
  },
];
      ...
      <section className="w-full border-t border-b bg-[var(--hero-bg)]" style={{ borderColor: "var(--hero-border)" }}>
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            식사 대용은 단백질 숫자만 보고 결정하면 실패하기 쉽습니다. 포만감, 칼로리, 다음 끼니까지 버티는지까지 같이 봐야 실제로 맞는 제품을 찾을 수 있습니다.
          </p>
        </div>
      </section>
