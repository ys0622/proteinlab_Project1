...
const roleCards = [
  ...
  },
  {
    ...
    linkLabel: "면역·호르몬과 단백질 더 자세히 보기",
  },
  {
    icon: "🧪",
    title: "호르몬·효소 | 몸의 신호와 대사를 조절",
    body: "인슐린, 성장호르몬, 글루카곤은 모두 단백질 호르몬입니다. 소화를 돕는 트립신·펩신도 단백질입니다. 단백질 공급이 부족하면 이들의 합성이 감소합니다.",
    href: "/guides/basics/immunity-hormone",
    linkLabel: "면역·호르몬과 단백질 더 자세히 보기",
  },
];
      ...
      >
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            단백질은 단순히 근육을 키우는 영양소가 아닙니다. 면역세포를 만들고, 호르몬을
            합성하고, 온몸의 조직을 유지하는 데도 단백질이 필요합니다.
          </p>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-[1200px] px-4 py-8 md:px-6">
        <div className="space-y-6">
          <section className="rounded-2xl border border-[#e8e6e3] bg-[#fffdf8] px-5 py-5">
            ...
            <Callout>
              아미노산이 결합한 단백질은 근육·피부·혈액·효소·호르몬 등 인체 거의 모든 구성
              성분의 원료입니다. 체내에서는 끊임없이 단백질이 생성되고 분해되며, 이를 단백질
              대사(아미노산 풀)라고 합니다.
            </Callout>
          ...
          <section className="rounded-2xl border border-[#e8e6e3] bg-[#fffdf8] px-5 py-5">
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-3">
              {roleCards.map((card) => (
                <article
                  key={card.title}
                  className="rounded-2xl border border-[#e8e6e3] bg-white px-4 py-4"
                >
                  <p className="text-2xl" aria-hidden="true">
                    {card.icon}
                  </p>
                  <h3 className="mt-3 text-base font-bold text-[var(--foreground)]">{card.title}</h3>
          ...
          <section className="rounded-2xl border border-[#e8e6e3] bg-[#fffdf8] px-5 py-5">
            ...
            <p className="mt-4 text-xs text-[var(--foreground-muted)]">
              출처: WHO·한국영양학회(일반 성인 기준), ISSN Position Stand(운동자 기준)
            </p>
          </section>

          <section className="rounded-2xl border border-[#e8e6e3] bg-white px-5 py-5 text-center">
            <Link
              href="/recommend"
