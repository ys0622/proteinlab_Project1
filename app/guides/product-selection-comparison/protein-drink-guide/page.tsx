...
const checklistRows = [
  ["단백질 함량", "한 병당 20g 전후인지 먼저 확인", "운동 후 보충인지 식사 보완인지 1차 판단이 빨라집니다."],
  ["당류", "체중 관리 목적이면 가급적 먼저 체크", "단백질이 높아도 당류가 높으면 실제 활용 목적이 달라질 수 있습니다."],
  ["칼로리", "식사 보완형인지 가벼운 보충형인지 구분", "칼로리가 낮으면 가벼운 보충, 높으면 식사 보완형에 가깝습니다."],
  ["단백질 밀도", "비슷한 칼로리 안에서 최종 비교", "같은 열량에서 어느 제품이 더 효율적인지 판단할 때 유용합니다."],
];

const drinkTypes = [
  {
    title: "밸런스형 RTD",
    body: "포만감이 있어 식사 보완으로 연결하기 쉽습니다. 당류와 칼로리를 함께 봐야 실제 목적에 맞는지 판단할 수 있습니다.",
  },
  {
    title: "워터형 RTD",
    body: "운동 중간이나 직후에 가볍게 마시기 좋습니다. 상대적으로 저칼로리 제품이 많아 체중 관리용 보충으로도 자주 활용됩니다.",
  },
  {
    title: "고단백 보충형",
    body: "30g 전후 고단백 제품은 운동 후 보충에 강점이 있지만, 칼로리와 당류가 같이 높지 않은지 반드시 확인해야 합니다.",
  },
];
...
const flowCards = [
  ...
  {
    title: "식사 보완",
    body: "칼로리와 포만감이 먼저입니다. 그다음 단백질 함량을 비교해야 식사 보완 목적과 맞는지 판단이 쉽습니다.",
  },
  {
    title: "체중 관리",
    body: "당류와 칼로리를 먼저 좁힌 뒤 마지막에 단백질 밀도와 단백질 g를 비교하는 편이 실수가 적습니다.",
  },
];

const comparisonSteps = [
  { title: "1차", body: "유형부터 나누기", note: "워터형 / 밸런스형 / 식사보완형" },
  { title: "2차", body: "당류·칼로리 거르기", note: "체중 관리용인지 식사 보완용인지 구분" },
  { title: "3차", body: "단백질 함량 비교", note: "20g 전후, 30g 이상 등 목적별 기준" },
  { title: "4차", body: "단백질 밀도 확인", note: "비슷한 칼로리끼리 마지막 비교" },
];
      ...
      <section className="w-full border-t border-b bg-[var(--hero-bg)]" style={{ borderColor: "var(--hero-border)" }}>
        <div className="mx-auto max-w-[1200px] px-4 py-5 md:px-6 md:py-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-muted)]">
            단백질 음료는 무엇부터 비교해야 할지 모르면 제품이 많아질수록 선택이 흔들리기 쉽습니다.
            <br />
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-[28px] border border-[#e2ebe4] bg-white px-5 py-5 shadow-[0_18px_50px_rgba(20,32,24,0.04)]">
            ...
            <blockquote className="mt-5 rounded-xl border border-[#dce8df] bg-[#f7fbf8] px-4 py-4 text-sm leading-6 text-[var(--foreground-muted)]">
              비교에서 중요한 것은 숫자 하나가 아니라 조합입니다. 단백질이 높아도 당류나 칼로리가 높으면 실제 용도는 완전히 달라질 수 있습니다.
            </blockquote>
          </section>
        </div>
      </main>
