  ...
  return (
    <>
      <header className="sticky top-0 z-50" style={{ background: "#16412D" }}>
        <div className="mx-auto flex h-14 max-w-[1180px] items-center px-4 md:h-16 md:px-5">
          <Link
            href="/"
        ...
        {open ? (
          <nav
            className="absolute inset-x-0 top-full z-50 max-h-[calc(100vh-3.5rem)] overflow-y-auto border-t border-white/10 bg-white px-4 py-2 shadow-lg md:hidden"
            aria-label="모바일 메뉴"
          >
            {visibleNavItems.map((item) => {
              if (item.children) {
