    ...
    <div>
      <div className="max-w-3xl">
        ...
        <p className="mt-2 text-sm leading-6 text-[var(--foreground-muted)] md:text-base">
          브랜드명이나 제품명으로 검색하면 제품 상세를 보고, 바로 비교 페이지나 추천 흐름까지
          이어서 볼 수 있습니다.
        </p>
      </div>
      ...
      <div className="relative mb-6 mt-6">
        ...
        <input
          ...
          className="w-full rounded-xl border border-[#e8e6e3] bg-white py-2 pl-11 pr-4 text-sm focus:border-[var(--accent)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)] md:py-3"
          autoComplete="off"
        />
        {query ? (
          <button
      ...
      {loaded && !trimmed ? (
        <div className="space-y-4">
          <div className="rounded-2xl border border-[#ebe5dc] bg-[#fcfaf6] px-5 py-10 text-center text-sm text-[var(--foreground-muted)]">
            브랜드명이나 제품명을 입력해보세요. 예: 셀렉스, 하이뮨, 랩노쉬, 테이크핏
          </div>
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {START_LINKS.map((link) => (
              <TrackedLink
                ...
                trackingSection="search_start_links"
      ...
      {loaded && trimmed && totalCount === 0 ? (
        <div className="py-16 text-center">
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            <TrackedLink
              href="/recommend"
              trackingLabel="검색 결과 없음에서 맞춤 추천 받기"
              trackingSection="search_empty_state"
              trackingPageType="search"
              className="inline-flex min-w-[124px] items-center justify-center rounded-full bg-[var(--accent)] px-4 py-2 text-xs font-semibold text-white shadow-[0_10px_24px_rgba(36,84,61,0.16)] transition-all hover:-translate-y-0.5 hover:opacity-95"
            >
              맞춤 추천 받기
            </TrackedLink>
            <TrackedLink
              href="/products"
              trackingLabel="검색 결과 없음에서 전체 제품 보기"
              trackingSection="search_empty_state"
              trackingPageType="search"
              className="rounded-full border border-[#d8d5d0] bg-white px-3.5 py-2 text-xs font-semibold text-[var(--foreground)] transition-colors hover:bg-[var(--accent-light)]"
            >
              전체 제품 보기
            </TrackedLink>
          </div>
